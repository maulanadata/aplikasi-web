<div id="content1">
	...
	<FORM CLASS="form-inline" METHOD="POST" id="tombol-tambah-container">
		<a href="pages/gagal-cetak.php" class="tombol">Cetak Siswa Tidak Diterima</a>
	</FORM>
...
<table id="tabel-tampil">
	...
	$q_tampil_daftar = mysqli_query($db, "SELECT * FROM tbuser WHERE status='Tidak DIterima'");
	...
	<tr>
		...
		<td>
			<div style='padding:2px' class="tombol-opsi-container"><a target="_blank" href="pages/gagal-kartu.php?id=<?php echo $r_tampil_daftar['id_user'];?>" class="tombol">Cetak Kartu</a></div>
			<div style='padding:2px' class="tombol-opsi-container"><a href="index.php?page=gagal-edit&id=<?php echo $r_tampil_daftar['id_user'];?>" class="tombol">Edit</a></div>
			<div style='padding:2px' class="tombol-opsi-container"><a href="proses/hapus.php?id=<?php echo $r_tampil_daftar['id_user']; ?>" onclick = "return confirm ('Apakah Anda Yakin Akan Menghapus Data Ini?')" class="tombol">Hapus</a></div>
		</td>
	</tr>
	<?php $nomor++; } 
	}?>
...
<?php
	$jml = mysqli_num_rows(mysqli_query($db, "SELECT * FROM tbuser WHERE status='Tidak DIterima'"));
...
<?php
	...
	for($i=1; $i<=$jml_page; $i++){
		if($i != $page){
			echo "<a href=\"?hal=siswa-gagal&page=$i\">$i</a>";
		}
		else {
