<div id="content1">
	...
	<FORM CLASS="form-inline" METHOD="POST" id="tombol-tambah-container">
		<a href="pages/semua-cetak.php" class="tombol">Cetak Semua Siswa</a>
	</FORM>
...
<table id="tabel-tampil">
	...
	$q_tampil_daftar = mysqli_query($db, "SELECT * FROM tbuser");
	if(mysqli_num_rows($q_tampil_daftar)>0)
	{
	while($r_tampil_daftar=mysqli_fetch_array($query)){
...
<?php
	$jml = mysqli_num_rows(mysqli_query($db, "SELECT * FROM tbuser"));
...
<?php
	...
	for($i=1; $i<=$jml_page; $i++){
		if($i != $page){
			echo "<a href=\"?hal=siswa-semua&page=$i\">$i</a>";
		}
		else {
