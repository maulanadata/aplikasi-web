<div id="content1">
	...
	<FORM CLASS="form-inline" METHOD="POST" id="tombol-tambah-container">
		<a href="pages/diterima-cetak.php" class="tombol">Cetak Siswa Diterima</a>
	</FORM>
...
<table id="tabel-tampil">
	...
	$q_tampil_daftar = mysqli_query($db, "SELECT * FROM tbuser WHERE status='Diterima'");
	...
	<tr>
		...
		<td>
			<div style='padding:2px' class="tombol-opsi-container"><a target="_blank" href="pages/diterima-kartu.php?id=<?php echo $id_user;?>" class="tombol">Cetak Kartu</a></div>
			<div style='padding:2px' class="tombol-opsi-container"><a href="index.php?page=diterima-edit&id=<?php echo $id_user;?>" class="tombol">Edit</a></div>
			<div style='padding:2px' class="tombol-opsi-container"><a href="proses/hapus.php?id=<?php echo $id_user; ?>" onclick = "return confirm ('Apakah Anda Yakin Akan Menghapus Data Ini?')" class="tombol">Hapus</a></div>
		</td>
	</tr>
	<?php $nomor++; } 
	}?>
...
<?php
	$jml = mysqli_num_rows(mysqli_query($db, "SELECT * FROM tbuser WHERE status='Diterima'"));
...
<?php
	...
	for($i=1; $i<=$jml_page; $i++){
		if($i != $page){
			echo "<a href=\"?hal=siswa-diterima&page=$i\">$i</a>";
		}
		else {
